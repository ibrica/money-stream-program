...
pub mod money_stream_program {
    ...
    pub fn dummy(ctx: Context<Dummy>) -> ProgramResult {
        Ok(())
    }
    ...
    pub fn balance(ctx: Context<Get>,authority: Pubkey) -> Result<()> {
    ...
    pub fn withdraw(ctx: Context<Withdraw>, authority: Pubkey ) -> Result<()> {
        let session_account = &mut ctx.accounts.session_account;
        if session_account.benefiter != authority {
            return Err(ErrorCode::Unauthorized.into());
        }
